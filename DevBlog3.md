...
Bellow is shown `applyPicture()` method that is responsible for placing the picture that was chosen from galery in to the frame on the AR place. In method is created sprite from the texture is which is essentially the picture chosen from gallery. The reason for using Sprite for placing the picture into the frame instead of other methods such as Raw Image is that the frame component is 3D object and RawImage component can not be used only for 2D objects.

Pictures that can be uploaded to the frame can have all possible sizes and ratios. Moreover those pictures can be placed to huge range of frames. Because of that, the `applyPicture()` must handle all of those scanarios. For that are used sprite renderer bounds of frame object and newly created Sprite from gallery picture. Those two values are divided and the result is scale factor. After that are transform values x and y multiplied by that scale factor. The result is that image is either scaled down or up, such as it fits to the frame. In method are conditions to check if the frame is either landscape of portrait. The reason for checking orientation of frame is that scaleFactor is calculated by using sprite bounds of another axis in each of those scenarios. So, for example in case of landscape we are using `bounds.size.y` and in case of portrait  `bounds.size.x`. Moreover the methods is checking if the picture itself is either landscape of portrait. Depening on that is scale factor multiplied by different value. So, in case of landscape we want to scale slightly less than in case of portrait.
